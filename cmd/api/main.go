...
// corsMiddleware permite que el navegador haga peticiones a nuestra API
// desde cualquier origen (necesario para que React pueda conectarse)
func corsMiddleware(next http.HandlerFunc) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		// Permitimos peticiones desde cualquier dominio
		w.Header().Set("Access-Control-Allow-Origin", "*")
		w.Header().Set("Access-Control-Allow-Methods", "GET, POST, OPTIONS")
		w.Header().Set("Access-Control-Allow-Headers", "Content-Type")

		// OPTIONS es una petición previa que hace el navegador para verificar CORS
...
func main() {
	...
	http.HandleFunc("/jobs", corsMiddleware(jobsHandler))
	http.HandleFunc("/jobs/", corsMiddleware(jobByIDHandler))
